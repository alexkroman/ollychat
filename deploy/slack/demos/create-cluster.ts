...
const CLUSTER_NAME = process.env.CLUSTER_NAME || "online-boutique";
...
// Step 3: Create GKE Cluster (Much simpler than standard GKE)
if (shell.exec(`gcloud container clusters create ${CLUSTER_NAME} \
  --region ${REGION} \
  --machine-type e2-small \
  --num-nodes 1 \
  --enable-ip-alias   
`).code !== 0) {
    console.error("❌ Failed to create GKE cluster");
    process.exit(1);
}
...
console.log("✅ GKE cluster created successfully!");