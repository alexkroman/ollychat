export const examples = [
    ...
    },
    {
      ...
      "answer": "irate(node_pressure_memory_waiting_seconds_total{instance=\"$node\",job=\"$job\"}[$__rate_interval])"
    },
    {
      "question": "What is the rate of IO pressure on the node?",
      "answer": "irate(node_pressure_io_waiting_seconds_total{instance=\"$node\",job=\"$job\"}[$__rate_interval])"
    },
    {
      "question": "What is the CPU usage percentage on the node?",
      "answer": "100 * (1 - avg(rate(node_cpu_seconds_total{mode=\"idle\", instance=\"$node\"}[$__rate_interval])))"
    },
    {
      "question": "What is the 1-minute load average percentage on the node?",
      "answer": "scalar(node_load1{instance=\"$node\",job=\"$job\"}) * 100 / count(count(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}) by (cpu))"
    },
    {
      "question": "What percentage of total memory is in use?",
      "answer": "((node_memory_MemTotal_bytes{instance=\"$node\", job=\"$job\"} - node_memory_MemFree_bytes{instance=\"$node\", job=\"$job\"}) / node_memory_MemTotal_bytes{instance=\"$node\", job=\"$job\"}) * 100"
    },
    {
      "question": "What percentage of memory is unavailable?",
      "answer": "(1 - (node_memory_MemAvailable_bytes{instance=\"$node\", job=\"$job\"} / node_memory_MemTotal_bytes{instance=\"$node\", job=\"$job\"})) * 100"
    },
    {
      "question": "What percentage of swap memory is in use?",
      "answer": "((node_memory_SwapTotal_bytes{instance=\"$node\",job=\"$job\"} - node_memory_SwapFree_bytes{instance=\"$node\",job=\"$job\"}) / (node_memory_SwapTotal_bytes{instance=\"$node\",job=\"$job\"})) * 100"
    },
    {
      "question": "What is the percentage of used filesystem space on the root mount?",
      "answer": "100 - ((node_filesystem_avail_bytes{instance=\"$node\",job=\"$job\",mountpoint=\"/\",fstype!=\"rootfs\"} * 100) / node_filesystem_size_bytes{instance=\"$node\",job=\"$job\",mountpoint=\"/\",fstype!=\"rootfs\"})"
    },
    {
      "question": "How many CPU cores are available on the node?",
      "answer": "count(count(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}) by (cpu))"
    },
    {
      "question": "How long has the node been running since the last boot?",
      "answer": "node_time_seconds{instance=\"$node\",job=\"$job\"} - node_boot_time_seconds{instance=\"$node\",job=\"$job\"}"
    },
    {
      "question": "What is the total size of the root filesystem?",
      "answer": "node_filesystem_size_bytes{instance=\"$node\",job=\"$job\",mountpoint=\"/\",fstype!=\"rootfs\"}"
    },
    {
      "question": "What is the total memory available on the node?",
      "answer": "node_memory_MemTotal_bytes{instance=\"$node\",job=\"$job\"}"
    },
    {
      "question": "What is the total swap memory available on the node?",
      "answer": "node_memory_SwapTotal_bytes{instance=\"$node\",job=\"$job\"}"
    },
    {
      "question": "What is the average system CPU usage across all cores?",
      "answer": "sum(irate(node_cpu_seconds_total{instance=\"$node\",job=\"$job\", mode=\"system\"}[$__rate_interval])) / scalar(count(count(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}) by (cpu)))"
    },
    {
      "question": "What is the average user CPU usage across all cores?",
      "answer": "sum(irate(node_cpu_seconds_total{instance=\"$node\",job=\"$job\", mode=\"user\"}[$__rate_interval])) / scalar(count(count(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}) by (cpu)))"
    },
    {
      "question": "What is the average IO wait CPU usage across all cores?",
      "answer": "sum(irate(node_cpu_seconds_total{instance=\"$node\",job=\"$job\", mode=\"iowait\"}[$__rate_interval])) / scalar(count(count(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}) by (cpu)))"
    },
    {
      "question": "What is the average IRQ and softIRQ CPU usage across all cores?",
      "answer": "sum(irate(node_cpu_seconds_total{instance=\"$node\",job=\"$job\", mode=~\".*irq\"}[$__rate_interval])) / scalar(count(count(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}) by (cpu)))"
    },
    {
      "question": "What is the average CPU usage across all cores for other modes?",
      "answer": "sum(irate(node_cpu_seconds_total{instance=\"$node\",job=\"$job\",  mode!='idle',mode!='user',mode!='system',mode!='iowait',mode!='irq',mode!='softirq'}[$__rate_interval])) / scalar(count(count(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}) by (cpu)))"
    },
    {
      "question": "What is the average idle CPU usage across all cores?",
      "answer": "sum(irate(node_cpu_seconds_total{instance=\"$node\",job=\"$job\", mode=\"idle\"}[$__rate_interval])) / scalar(count(count(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}) by (cpu)))"
    },
    {
      "question": "How much memory is actively in use (excluding cached and buffered)?",
      "answer": "node_memory_MemTotal_bytes{instance=\"$node\",job=\"$job\"} - node_memory_MemFree_bytes{instance=\"$node\",job=\"$job\"} - (node_memory_Cached_bytes{instance=\"$node\",job=\"$job\"} + node_memory_Buffers_bytes{instance=\"$node\",job=\"$job\"} + node_memory_SReclaimable_bytes{instance=\"$node\",job=\"$job\"})"
    },
    {
      "question": "How much memory is used for caching and buffers?",
      "answer": "node_memory_Cached_bytes{instance=\"$node\",job=\"$job\"} + node_memory_Buffers_bytes{instance=\"$node\",job=\"$job\"} + node_memory_SReclaimable_bytes{instance=\"$node\",job=\"$job\"}"
    },
    {
      "question": "How much free memory is available on the node?",
      "answer": "node_memory_MemFree_bytes{instance=\"$node\",job=\"$job\"}"
    },
    {
      "question": "How much swap memory is currently used?",
      "answer": "(node_memory_SwapTotal_bytes{instance=\"$node\",job=\"$job\"} - node_memory_SwapFree_bytes{instance=\"$node\",job=\"$job\"})"
    },
    {
        "question": "What is the current network receive rate in bits per second for a specific node and job?",
        "answer": "irate(node_network_receive_bytes_total{instance=\"$node\",job=\"$job\"}[$__rate_interval])*8"
      },
      {
        "question": "What is the current network transmit rate in bits per second for a specific node and job?",
        "answer": "irate(node_network_transmit_bytes_total{instance=\"$node\",job=\"$job\"}[$__rate_interval])*8"
      },
      {
        "question": "What percentage of filesystem space is used on non-root filesystems for a specific node and job?",
        "answer": "100 - ((node_filesystem_avail_bytes{instance=\"$node\",job=\"$job\",device!~'rootfs'} * 100) / node_filesystem_size_bytes{instance=\"$node\",job=\"$job\",device!~'rootfs'})"
      },
      {
      ...
      },
      {
        ...
        "answer": "sum(irate(node_cpu_seconds_total{instance=\"$node\",job=\"$job\", mode=\"nice\"}[$__rate_interval])) / scalar(count(count(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}) by (cpu)))"
      },
      {
        "question": "What is the iowait CPU usage rate for a specific node and job?",
        "answer": "sum by(instance) (irate(node_cpu_seconds_total{instance=\"$node\",job=\"$job\", mode=\"iowait\"}[$__rate_interval])) / scalar(count(count(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}) by (cpu)))"
      },
      {
      ...
      },
      {
        ...
        "answer": "sum(irate(node_cpu_seconds_total{instance=\"$node\",job=\"$job\", mode=\"softirq\"}[$__rate_interval])) / scalar(count(count(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}) by (cpu)))"
      },
      {
        "question": "What is the steal CPU usage rate for a specific node and job?",
        "answer": "sum(irate(node_cpu_seconds_total{instance=\"$node\",job=\"$job\", mode=\"steal\"}[$__rate_interval])) / scalar(count(count(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}) by (cpu)))"
      },
      {
      ...
      },
      {
        ...
        "answer": "node_memory_MemTotal_bytes{instance=\"$node\",job=\"$job\"} - node_memory_MemFree_bytes{instance=\"$node\",job=\"$job\"} - node_memory_Buffers_bytes{instance=\"$node\",job=\"$job\"} - node_memory_Cached_bytes{instance=\"$node\",job=\"$job\"} - node_memory_Slab_bytes{instance=\"$node\",job=\"$job\"} - node_memory_PageTables_bytes{instance=\"$node\",job=\"$job\"} - node_memory_SwapCached_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the size of page tables in bytes for a specific node and job?",
        "answer": "node_memory_PageTables_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the size of cached swap memory in bytes for a specific node and job?",
        "answer": "node_memory_SwapCached_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the size of slab memory in bytes for a specific node and job?",
        "answer": "node_memory_Slab_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the size of cached memory in bytes for a specific node and job?",
        "answer": "node_memory_Cached_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the size of buffer memory in bytes for a specific node and job?",
        "answer": "node_memory_Buffers_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
      ...
      },
      {
        ...
        "answer": "node_memory_HardwareCorrupted_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the total size of used disk space in bytes for non-root filesystems for a specific node and job?",
        "answer": "node_filesystem_size_bytes{instance=\"$node\",job=\"$job\",device!~'rootfs'} - node_filesystem_avail_bytes{instance=\"$node\",job=\"$job\",device!~'rootfs'}"
      },
      {
        "question": "What is the rate of disk reads completed per second for specific disk devices and a specific node and job?",
        "answer": "irate(node_disk_reads_completed_total{instance=\"$node\",job=\"$job\",device=~\"$diskdevices\"}[$__rate_interval])"
      },
      {
        "question": "What is the rate of disk writes completed per second for specific disk devices and a specific node and job?",
        "answer": "irate(node_disk_writes_completed_total{instance=\"$node\",job=\"$job\",device=~\"$diskdevices\"}[$__rate_interval])"
      },
      {
        "question": "What is the rate of disk read bytes per second for specific disk devices and a specific node and job?",
        "answer": "irate(node_disk_read_bytes_total{instance=\"$node\",job=\"$job\",device=~\"$diskdevices\"}[$__rate_interval])"
      },
      {
        "question": "What is the rate of disk write bytes per second for specific disk devices and a specific node and job?",
        "answer": "irate(node_disk_written_bytes_total{instance=\"$node\",job=\"$job\",device=~\"$diskdevices\"}[$__rate_interval])"
      },
      {
        "question": "What is the disk IO time rate in seconds per second for specific disk devices and a specific node and job?",
        "answer": "irate(node_disk_io_time_seconds_total{instance=\"$node\",job=\"$job\",device=~\"$diskdevices\"} [$__rate_interval])"
      },
      {
        "question": "What is the percentage of user mode guest CPU usage for a specific node and job?",
        "answer": "sum by(instance) (irate(node_cpu_guest_seconds_total{instance=\"$node\",job=\"$job\", mode=\"user\"}[1m])) / on(instance) group_left sum by (instance)((irate(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}[1m])))"
      },
      {
        "question": "What is the percentage of nice mode guest CPU usage for a specific node and job?",
        "answer": "sum by(instance) (irate(node_cpu_guest_seconds_total{instance=\"$node\",job=\"$job\", mode=\"nice\"}[1m])) / on(instance) group_left sum by (instance)((irate(node_cpu_seconds_total{instance=\"$node\",job=\"$job\"}[1m])))"
      },
      {
        "question": "What is the size of inactive memory in bytes for a specific node and job?",
        "answer": "node_memory_Inactive_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the size of active memory in bytes for a specific node and job?",
        "answer": "node_memory_Active_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the size of committed memory in bytes for a specific node and job?",
        "answer": "node_memory_Committed_AS_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the memory commit limit in bytes for a specific node and job?",
        "answer": "node_memory_CommitLimit_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the size of inactive file memory in bytes for a specific node and job?",
        "answer": "node_memory_Inactive_file_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the size of inactive anonymous memory in bytes for a specific node and job?",
        "answer": "node_memory_Inactive_anon_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the size of active file memory in bytes for a specific node and job?",
        "answer": "node_memory_Active_file_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
        "question": "What is the size of active anonymous memory in bytes for a specific node and job?",
        "answer": "node_memory_Active_anon_bytes{instance=\"$node\",job=\"$job\"}"
      },
      {
